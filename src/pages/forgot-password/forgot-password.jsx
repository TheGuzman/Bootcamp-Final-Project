...
function ForgotPasswordPage() {
  ...
  function handlePasswordChange(e) {
    ...
    if (userPassword !== userPasswordConfirmation) {
      ...
      fetch(`${url}/auth/validate-new-password`, options).then((r) => {
        r.json();
        if (r.status === 404) {
          setLoading(false);
          setError(true);
      ...
      <form onSubmit={handlePasswordChange}>
        ...
        >
          ...
          <Button variant="contained" color="secondary" type="submit">
            {t("forgotPasswordPage.updatePasswordButton")}
          </Button>
        </Box>
      </form>
      {isSumbitted !== false ? (
        loading !== true ? (
          error !== false ? (
            <Typography sx={{ margin: "1.5em 1em 1em 1em" }} textAlign={"center"} color="error.main" variant="h6">
              {" "}
              {t("forgotPasswordPage.fail.updateErrorMsg")}
            </Typography>
          ) : (
            <Typography sx={{ margin: "1.5em 1em 1em 1em" }} textAlign={"center"} color="success.main" variant="h6">
              {t("forgotPasswordPage.success.updateMsg")}
            </Typography>
          )
        ) : (
