...
export default function RegisterPage() {
  ...
  function handleSubmit(e) {
    ...
    if (userName.length >= 4 && userPasswordConfirmation === userPassword && isEmail(userEmail)) {
      ...
      fetch(`${url}/auth/register`, options).then((r) => {
        if (r.status === 409) {
          setError(true);
          setLoading(false);
        } else {
      ...
      {isSumbitted === false ? (
        <Box sx={{ display: "flex", justifyContent: "center", flexDirection: "column", alignItems: "center" }}>
          <Typography sx={{ margin: "1em" }} variant="h5">
            {t("registrationPage.registrationMessage")}
          </Typography>
        ...
        isLoading === true ? (
          <Typography textAlign={"center"} variant="h4">
            {t("registrationPage.registrationPending")}
          </Typography>
        ) : (
          <Box sx={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
            ...
            <a href="/register" component={Link}>
              {t("registrationPage.goBackLink")}
            </a>
          </Box>
        )
      ) : (
