...
import {
  ...
  SenderName,
  TextArea,
} from "../../components/styled-chat/styled-chat.jsx";
  ...
  return (
    <Stack>
      <Page>
        ...
        >
          <Stack sx={{ width: "30%" }}>
            <Typography variant="h5" textAlign={"center"} sx={{ fontFamily: "BrainFish" }}>
              {t("joinFishbowlPage.fishbowllName")}
            </Typography>
          ...
          <Stack sx={{ width: "30%" }}>
            <Typography variant="h5" textAlign={"center"} sx={{ fontFamily: "BrainFish" }}>
              {t("joinFishbowlPage.fishbowlDescription")}
            </Typography>
            <Typography variant="h6" textAlign={"center"}>
              {fishbowlInfo?.description}
            </Typography>
          </Stack>
          <Stack sx={{ width: "30%" }}>
            <Typography variant="h5" textAlign={"center"} sx={{ fontFamily: "BrainFish" }}>
              {t("joinFishbowlPage.fishbowlCreator")}
            </Typography>
            <Typography variant="h6" textAlign={"center"}>
              {fishbowlInfo?.creator}
            </Typography>
          </Stack>
        </Stack>
        <Stack>
          <Typography variant="h3" textAlign={"center"} sx={{ fontFamily: "BrainFish" }}>
            {t("joinFishbowlPage.activeFishes")}
          </Typography>
          <Stack direction="row" justifyContent={"center"} flexWrap={"wrap"} columnGap={4}>
            {users.map((e, i) => (
              <Chip key={i} color="info" label={e} icon={<Icon icon="ion:fish-sharp" width="20" height="20" />} />
            ))}
          </Stack>
        </Stack>
        <Stack>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <SendButton onKeyPress={handleKeypress} type="submit">
            {t("buttons.submit")}
          </SendButton>
        </Form>
      </Page>
